...
for (i in 1:num_region)
{
  ...
  index_causal_frame = rbind(index_causal_frame, c(min(origin.startind + signalpos - 1), max(origin.startind + signalpos - 1)))
...
for (setc in 1:length(cv))
{
  ...
  Simu_dBiRS = function(s)
  {
    ...
    for (part in 1:nrow(Block.ind))
    {
      ...
      res_dBiRS = dBiRS_Generator(phenotype = phenotype, X = XTrans, GenoPrefix = GenoPrefix, Block.ind = ind.part,
                                  sample_index = select_sample, IfScale = F, family = 'gaussian', trunc = 6, MB = 1000, alpha = 0.05)

      save(list = c('res_dBiRS'), file = paste0('result_dbirs_c1/dBiRS_c = ', c, 's = ', s, 'part', part, '.RData'))
    }
  }
  ...
  stopCluster(cl)
  bb = Sys.time()
  print(paste0('Block BiRS Detection', 'c' = c, '---------------End, Time = ', bb - aa))
}
#################################################################################################################################################
...
for (setc in 1:length(cv))
{
  ...
  Simu_dBiRS = function(s)
  {
    ...
    for (part in 1:nrow(Block.ind))
    {
      ind.part = Block.ind[part, ]
      res_dBiRS = dBiRS_Generator(phenotype = phenotype, X = XTrans, GenoPrefix = GenoPrefix, Block.ind = ind.part,
                                  sample_index = select_sample, IfScale = F, family = 'binomial', trunc = 6, MB = 1000, alpha = 0.05)

      save(list = c('res_dBiRS'), file = paste0('result_dbirs_b1/dBiRS_c = ', c, 's = ', s, 'part', part, '.RData'))
    }
  }
