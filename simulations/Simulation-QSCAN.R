...
for (setc in 1:length(cv))
{
  ...
  for(s in 1:nsimu)
  {
    ...
    Simu_QSCAN = function(part)
    {
      ...
      save(list = c('res_QSCAN'), file = paste0('result_qscan_c1/QSCAN_c = ', c, 's = ', s, 'part', part, '.RData'))
    }
    ...
    RES = foreach(part = 1:100, .packages = c("BiRS", "MASS", "QSCAN", "Matrix", "seqminer", "genio", "KnockoffScreen")) %dopar% Simu_QSCAN(part)
    
    stopImplicitCluster()
    stopCluster(cl)
    bb = Sys.time()
    
    print(paste0('s = ', s, '----end, time = ', bb - aa))
  }
}
###############################################################################################################################################################
rm(list = ls())
...
for (setc in 1:length(cv))
{
  ...
  for(s in 1:nsimu)
  {
    ...
    Simu_QSCAN = function(part)
    {
      ...
      save(list = c('res_QSCAN'), file = paste0('result_qscan_b1/QSCAN_c = ', c, 's = ', s, 'part', part, '.RData'))
    }
